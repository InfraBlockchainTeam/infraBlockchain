...
```
infra-cli set account permission addressbook active --add-code
```
...
```
infrablockchain-cpp -o addressbook.wasm addressbook.cpp --abigen
```

Smart contracts on InfraBlockchain are upgradeable so the contract can be redeployed with changes.
...
```
infra-cli push action addressbook upsert '["alice", "alice", "liddell", 21, "123 drink me way", "wonderland", "amsterdam"]' -p alice@active
```

`infra-cli` will return some data, that includes all the actions executed in the transaction
...
* [Inline Actions to External Contracts](inline-actions-to-external-contracts.md): Learn how to construct actions and send those actions to an external contract.
