...
```
infra-cli create account eosio hodl YOUR_PUBLIC_KEY
infrablockchain-cpp hodl.cpp -o hodl.wasm
infra-cli set contract hodl ./ -p hodl@active
```

As mentioned in a previous tutorial, this contract needs an eosio.code permission:

```
infra-cli set account permission hodl active --add-code
```

Next, create a testing account:

```
infra-cli create account eosio han DEVELOPMENT_KEY
```

Let's transfer some SYS tokens issued in the previous section to `han`:

```
infra-cli push action eosio.token transfer '[ "alice", "han", "100.0000 SYS", "Slecht geld verdrijft goed" ]' -p alice@active
```
