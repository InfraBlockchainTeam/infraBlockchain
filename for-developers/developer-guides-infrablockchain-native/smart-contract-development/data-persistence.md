...
```
infrablockchain-cpp addressbook.cpp -o addressbook.wasm
```
...
```
infra-cli create account eosio addressbook YOUR_PUBLIC_KEY -p eosio@active
```

Deploy the `addressbook` contract

```
infra-cli set contract addressbook CONTRACTS_DIR/addressbook -p addressbook@active
```
...
```
infra-cli push action addressbook upsert '["alice", "alice", "liddell", "123 drink me way", "wonderland", "amsterdam"]' -p alice@active
```
...
```
infra-cli push action addressbook upsert '["bob", "bob", "is a loser", "doesnt exist", "somewhere", "someplace"]' -p alice@active
```
...
```
infra-cli get table addressbook addressbook people --lower alice --limit 1
```
...
* [Secondary Indices](secondary-indices.md): Learn how to add another index to the `addressbook` contract that you created in the preceding **Data Persistence** section.
